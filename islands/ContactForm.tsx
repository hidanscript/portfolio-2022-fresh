...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const resp = await fetch(
      `/api/send-email?email=${data.email}&firstName=${data.firstName}&lastName=${data.lastName}&message=${data.message}`,
      { method: "POST" }
    );
    if (resp.status === 200) {
  ...
  return (
    <Container>
      <h3
        class={tw`text-2xl font-bold leading-7 mt-20 text-center sm:text-3xl sm:truncate`}
      >
        Contact
      </h3>
      {status.success && (
      ...
      <div
        class={tw`flex justify-center mt-10 p-10 rounded-xl shadow-md`}
        ...
        <form class={tw`w-full max-w-lg`} onSubmit={handleSubmit}>
          <div class={tw`flex flex-wrap -mx-3 mb-6`}>
            <div class={tw`w-full md:w-1/2 px-3 mb-6 md:mb-0`}>
              <label
                class={tw`block uppercase tracking-wide text-gray-200 text-xs mb-2" for="grid-last-name`}
              >
                First Name
              </label>
              <input
          ...
          <div class={tw`flex flex-wrap -mx-3 mb-6`}>
            <div class={tw`w-full px-3`}>
              <label
                class={tw`block uppercase tracking-wide text-gray-200 text-xs mb-2" for="grid-password`}
              >
                E-mail
              </label>
              <input
          ...
          <div class={tw`flex items-center justify-center`}>
            <button
              class={tw`shadow focus:shadow-outline focus:outline-none py-2 px-4 rounded-md border border-solid border-gray-200 w-full text-white ${
                !loading ? "hover:text-gray-900 hover:bg-gray-100" : ""
              }`}
              type="submit"
              disabled={loading}
              style={loading ? { opacity: 0.5 } : {}}
